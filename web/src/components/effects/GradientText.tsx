...
// Adapted from react-bits (reactbits.dev) GradientText — MIT + Commons Clause.
...
export const GradientText = memo(function GradientText({
  ...
  return (
    <motion.div
      className={`rb-gradient-text ${showBorder ? 'rb-with-border' : ''} ${className}`}
      onMouseEnter={handleMouseEnter}
      onMouseLeave={handleMouseLeave}
    >
      {showBorder && <motion.div className="rb-gradient-overlay" style={{ ...gradientStyle, backgroundPosition }} />}
      <motion.div className="rb-text-content" style={{ ...gradientStyle, backgroundPosition }}>
        {children}
      </motion.div>
    </motion.div>
  );
