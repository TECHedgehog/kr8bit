...
// Adapted from react-bits (reactbits.dev) GlowCursor — MIT + Commons Clause.
// https://github.com/DavidHDev/react-bits — snapshot of src/content, converted to TS.
// Deviation: pointer tracked on window (not the container) so the effect keeps
// its trail when mounted in the pointer-events: none global layer.
...
export const GlowCursor = memo(function GlowCursor({
  color = '#67E8F9',
  ...
  className = '',
  style,
}: GlowCursorProps): JSX.Element {
  const containerRef = useRef<HTMLDivElement | null>(null);
  const canvasRef = useRef<HTMLCanvasElement | null>(null);
  const propsRef = useRef<GlowCursorProps>({} as GlowCursorProps);
  ...
  useEffect(() => {
    ...
    const render = (now: number) => {
      ...
      const fadeTarget = initialized && config.enabled !== false && !shouldFade ? 1 : 0;
  ...
  return (
    <div ref={containerRef} className={`glow-cursor${className ? ` ${className}` : ''}`} style={style}>
