...
// Adapted from react-bits (reactbits.dev) GradientWaves — MIT + Commons Clause.
// https://github.com/DavidHDev/react-bits — snapshot of src/content, converted to TS.
// Deviation: pointer tracked on window (not the canvas) so the effect keeps
// its parallax when mounted in the pointer-events: none global layer.
...
const ctxMap = new WeakMap<HTMLDivElement, GradientContext>();
...
export const GradientWaves = memo(function GradientWaves({
  horizonColor = '#5227FF',
  waveColor = '#FF9FFC',
  crestColor = '#FFFFFF',
  speed = 0.4,
  ...
  grainIntensity = 0.05,
  className = '',
}: GradientWavesProps): JSX.Element {
