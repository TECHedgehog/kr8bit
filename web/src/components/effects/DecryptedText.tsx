...
// Adapted from react-bits (reactbits.dev) DecryptedText — MIT + Commons Clause.
...
export const DecryptedText = memo(function DecryptedText({
  ...
  const shuffleText = useCallback(
    (originalText: string, currentRevealed: Set<number>): string => {
      return originalText
        ...
        .map((char, i) => {
          if (char === ' ') return ' ';
          if (currentRevealed.has(i)) return originalText[i] ?? char;
  ...
  return (
    <motion.span className={parentClassName} ref={containerRef} style={styles} {...animateProps} {...props}>
