...
export interface GlassLensParams {
  ...
  lensH: number;
  borderRadius: number;
  /** Base short-axis displacement-map resolution used by the aspect-aware allocator. */
  mapSize: number;
...
export interface LensMapSize {
  width: number;
  height: number;
}

/**
 * Allocate samples by physical aspect ratio instead of stretching a square
 * raster across the lens. Small lenses stay cheap; wide lenses retain corner
 * detail without exceeding the renderer's practical limits.
 */
export const aspectAwareMapSize = (
  base: number,
  lensWidth: number,
  lensHeight: number,
): LensMapSize => {
  const width = Math.max(1, lensWidth);
  const height = Math.max(1, lensHeight);
  const shortPx = Math.min(width, height);
  const longPx = Math.max(width, height);
  const shortSamples = Math.max(base, 512);
  const maxDimension = 2048;
  const maxPixels = 2_097_152;
  const even = (value: number) => Math.max(2, value & ~1);
  let short = even(shortSamples);
  let long = even(Math.min(maxDimension, Math.max(short, Math.round((short * longPx) / shortPx))));
  if (short * long > maxPixels) {
    long = even(Math.floor(maxPixels / short));
  }
  if (width >= height) return { width: long, height: short };
 ...
 */
export const createLensMapGenerator = (
  mapWidth: number,
  mapHeight = mapWidth,
): LensMapGenerator => {
  ...
  return {
    generate(shape) {
      ...
      const halfWMap = mapWidth >> 1;
      const halfHMap = mapHeight >> 1;
      ...
      const stepX = (2 * halfW) / mapWidth;
      const stepY = (2 * halfH) / mapHeight;
      const invW = 1 / halfW;
      const invH = 1 / halfH;
      ...
      if (hasDome) {
        ...
        if (lutLen !== halfWMap) {
          domeLut = new Float32Array(halfWMap);
          lutLen = halfWMap;
          lutDirty = true;
        }
        if (lutDirty) {
          ...
          for (let col = 0; col < halfWMap; col += 1) {
      ...
      for (let row = 0; row < halfHMap; row += 1) {
        const mirrorRow = mapHeight - 1 - row;
        ...
        const mirrorRowBase = mirrorRow * mapWidth;
        for (let col = 0; col < halfWMap; col += 1) {
          const mirrorCol = mapWidth - 1 - col;
          const px = -((col + 0.5) * stepX - halfW);
